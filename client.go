...
func sendMessageMenu() {
	...
	kirimPesankeServer(message)
}

func kirimPesankeServer(message string) {
	serverConn, err := net.DialTimeout("tcp", "127.0.0.1:1127", 3*time.Second)
	if err != nil {
		panic(err)
	}
	...
	_, err = serverConn.Read(byteReply)
	if err != nil {
		panic(err)
	}
